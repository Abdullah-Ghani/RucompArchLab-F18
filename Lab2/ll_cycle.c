...
int ll_has_cycle(node *head) {
	/* your code here */
node * tortoise = head, * hare = head;/* creates the nodes tortoise and hare*/
  while (hare && hare->next && hare->next->next) {/* checks to make sure the next and next to next are not NULL*/
    hare = hare->next->next;/* increments by two node*/
    tortoise = tortoise->next;/*increments by one node*/
    if (tortoise == hare)/*if equal then they are cyclic else they are acyclic*/
      return 1;
  }
  return 0;
}
