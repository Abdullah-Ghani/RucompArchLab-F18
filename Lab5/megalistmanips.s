...
mapLoop: 
	...
	add	t1, t1, 4		# offset the array address by the count(This line was fixed) 
	lw	a0, 0(t1)		# load the value at that address into a0 
	 
	jalr	s1			# call the function on that value.(This line was fixed)
	 
	sw	a0, 0(t1)		# store the returned value back into the array 
	addi	t0, t0, 1		# increment the count 
	bne	t0, t2, mapLoop	# repeat if we haven't reached the array size yet 
	 
	lw	a0, 8(s0)		# load the address of the next node into a0. (This line was fixed)
	lw	a1, 0(s1)		# put the address of the function back into a1 to prepare for the recursion 
	 
	jal ra 	map			# recurse(this line was fixed)
